...
namespace SanteDB.Matcher.Transforms.Names
{
    ...
    /// </summary>
    [Description("Extract the specified portion out of a name (Given, Family, etc.)"), DisplayName("Name Part Extract")]
    [TransformArgument(typeof(String), "part", "The part to extract (Given, Prefix, Family, Suffix)", true)]
    public class NamePartExtractTransform : IUnaryDataTransformer
    {
        ...
        public object Apply(object input, params object[] parms)
        {
            ...
            if (en == null)
            {
                throw new ArgumentOutOfRangeException(nameof(input), "This transform requires an EntityName to be in scope");
            }
