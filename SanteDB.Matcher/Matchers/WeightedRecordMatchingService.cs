    ...
    public class WeightedRecordMatchingService : BaseRecordMatchingService
    {
        ...
        public override IEnumerable<IRecordMatchResult<T>> Classify<T>(T input, IEnumerable<T> blocks, string configurationName)
        {
            try
            {
                ...
                return blocks.AsParallel().AsOrdered().Select(b => this.ClassifyInternal(input, b, strongConfig.Scoring, strongConfig.ClassificationMethod, strongConfig.MatchThreshold, strongConfig.NonMatchThreshold)).ToList();
            }
            catch (Exception e)
            {
